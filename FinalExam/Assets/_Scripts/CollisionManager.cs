...
public class CollisionManager : MonoBehaviour
{
    ...
    private static void Reflect(BulletBehaviour s)
    {
        if ((s.collisionNormal == Vector3.forward) || (s.collisionNormal == Vector3.back))
        {
            ...
            // s.transform.position += s.direction  * Time.deltaTime;
            s.transform.position += s.direction;
        }
    ...
    public static void CheckAABBs(CubeBehaviour a, CubeBehaviour b)
    {
            ...
            (a.min.z <= b.max.z && a.max.z >= b.min.z))
        {
            ...
            if (b.gameObject.name == "Player" && a.gameObject.name != "Ground" && a.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.DYNAMIC)
            {
                a.gameObject.GetComponent<RigidBody3D>().velocity = b.gameObject.GetComponent<RigidBody3D>().velocity;
                a.transform.position -= face + a.gameObject.GetComponent<RigidBody3D>().velocity * Time.deltaTime;
            }


            if (b.gameObject.name == "Player" && a.gameObject.name != "Ground" && a.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.STATIC && a.gameObject.name == "BlockB" || a.gameObject.name == "BlockB(1)" || a.gameObject.name == "BlockB(2)" || a.gameObject.name == "BlockB(3)")
            {
                b.gameObject.transform.position += face * Time.deltaTime;
            }

            //if (b.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.STATIC && b.gameObject.name != "Ground")
            //{
            //    a.gameObject.transform.position += face * Time.deltaTime;

            //    a.isGrounded = true;
            //}
            //a.transform.position += face;
